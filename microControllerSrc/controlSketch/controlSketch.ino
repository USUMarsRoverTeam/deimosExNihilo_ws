...
void calcTempAndHumidity(int tempRaw, int humidityRaw);
...
void calcTempAndHumidity(int tempRaw, int humidityRaw){
  ...
  temp = (tempVoltage * 1000) * tempSlope + tempIntercept;

  // Calculating Humidity
  float humidityVoltage = humidityRaw * (3.3/1023.0);
  humidity = (humidityVoltage * 1000) * humiditySlope + humidityIntercept;
}
...
void loop(){
  ...
  calcTempAndHumidity(analogRead(TEMP_PIN), analogRead(HUMIDITY_PIN));
  temp_msg.data = temp;
