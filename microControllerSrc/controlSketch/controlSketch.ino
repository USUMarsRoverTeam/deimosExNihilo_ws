...
rcl_subscription_t sub_arm;     // Arm Controls (Position)
rcl_subsctiption_t sub_arm_vel; // Arm Controls (Velocity)
rcl_subscription_t sub_arm_home;// Arm Going Home 
...
void updateArmPos(){
  ...
  if (lastArmUpdate > 0) {
    float dt = (currentTime - lastArmUpdate) / 1000.0;  // Convert to seconds
    
    for (int i = 0; i < 6; i++){
      // Integrate velocity to get new position
      float newPos = current_arm_positions[i] + arm_velocities[i] * dt;
      
      // Clamp to bounds
      current_arm_positions[i] = constrain(newPos, armBounds[i][0], armBounds[i][1]);
      
      // Send to servo
      maestro.setTarget(i, current_arm_positions[i]);
    }
  }
...
void setup(){
  ...
  // Create executor
  const int num_handles = 7;  // 6 subscribers
...
void loop(){
  ...
  // Update Arm Positions from Velocities
  updateArmPos();

  // Calculating Temp and Humidity to Publish
